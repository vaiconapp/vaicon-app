...
const buildEpendStdHtml = (orders, summaryOrders) => {
  ...
  const orderBlock = (o) => {
    ...
    (o.coatings||[]).filter(n=>n&&String(n).trim()).forEach(name=>{
      ...
      const parts = [`<div style="font-weight:900;color:${color};font-size:30px;letter-spacing:0.5px;margin-bottom:4px;line-height:1.15">${escapeHtml(name)}</div>`];
      if (fyllo) parts.push(`<div style="margin-left:16px;font-size:25px;line-height:1.25;margin-bottom:2px"><b>Φύλλο:</b> ${fyllo}</div>`);
      if (perv)  parts.push(`<div style="margin-left:16px;font-size:25px;line-height:1.25;margin-bottom:2px"><b>Περβάζι:</b> ${perv}</div>`);
      if (type==='EXO' && kasa) parts.push(`<div style="margin-left:16px;font-size:25px;line-height:1.25;margin-bottom:2px"><b>Κάσα:</b> ${kasa}</div>`);
      if (type==='MESA' && d.pihaki) parts.push(`<div style="margin-left:16px;font-size:22px;line-height:1.25;color:#1565C0;font-weight:900;margin-top:3px">✓ Πηχάκι (ξυλογωνιά)</div>`);
      sections.push(parts.join(''));
    });
    const sectionsHtml = sections.join('<div style="border-top:2px dashed #999;margin:7px 0"></div>') || '<div style="font-size:22px;color:#777;font-style:italic">(χωρίς στοιχεία επένδυσης)</div>';
    const fora = o.side==='ΑΡΙΣΤΕΡΗ'?'ΑΡΙΣΤΕΡΗ':'ΔΕΞΙΑ';
    const installBadge = o.installation==='ΝΑΙ'
      ? `<span style="background:#1565C0;color:white;font-weight:900;font-size:24px;padding:4px 14px;border-radius:8px;margin-left:12px">ΜΟΝΤΑΡΙΣΜΑ</span>` : '';
    return `<div class="ord">
      <div class="ordno">${escapeHtml(o.orderNo||'—')}</div>
      <div class="ordbody">
        <div style="display:flex;align-items:center;gap:14px;border-bottom:3px solid #1a1a1a;padding-bottom:8px;margin-bottom:10px;flex-wrap:wrap">
          <div style="font-size:38px;font-weight:900;color:#1565C0">${escapeHtml(o.h||'—')} × ${escapeHtml(o.w||'—')}</div>
          <div style="font-size:28px;font-weight:900;color:#8B0000">${fora}</div>
          ${installBadge}
        </div>
        ${sectionsHtml}
      </div>
  ...
  const summaryList = (summaryOrders||[]).filter(orderHasMaterials);
  const summaryPage = summaryList.length ? `<div style="page-break-before:always">${buildMatSummaryBody(summaryList)}</div>` : '';
  return `<html><head><meta charset="utf-8"><style>
    ...
    .ordno { width: 18mm; margin-right:3mm; display:flex; align-items:center; justify-content:center;
             font-size:56px; font-weight:900; color:#1a1a1a; letter-spacing:2px;
             writing-mode: vertical-rl; transform: rotate(180deg); white-space:nowrap; }
    .ordbody { flex:1; min-width:0; }
    ${MAT_SUMMARY_CSS}
    * { -webkit-print-color-adjust:exact; print-color-adjust:exact; }
  </style></head><body>${pages}${summaryPage}</body></html>`;
};
...
const orderHasMaterials = (o) => { const t = materialTotals([o]); return t.coatings.length>0 || t.cases.length>0 || t.frameExo.length>0 || t.frameMesa.length>0 || t.pihaki>0; };

// ── Σύνολο υλικών: κοινό HTML (χρησιμοποιείται σε αυτόνομη εκτύπωση & ως τελευταία σελίδα στην εκτύπωση επενδύσεων) ──
const MAT_SUMMARY_CSS = `
  .matsum{font-family:Arial,sans-serif;color:#000;}
  .matsum .cols{column-count:2;column-gap:16px;}
  .matsum .sec{break-inside:avoid;-webkit-column-break-inside:avoid;margin-bottom:6px;}
  .matsum table{width:100%;border-collapse:collapse;font-size:11px;}
  .matsum td{padding:2px 5px;border-bottom:1px solid #bbb;}
  .matsum h1{font-size:14px;margin-bottom:2px;} .matsum h2.sub{font-size:10px;color:#555;margin:0 0 6px;}
  .matsum h3{font-size:11px;margin:6px 0 2px;border-bottom:1.5px solid #000;padding-bottom:1px;}
  .matsum .orders{margin-top:10px;padding-top:6px;border-top:1.5px solid #000;}
  .matsum .chips{display:flex;flex-wrap:wrap;gap:2px;margin-top:4px;}
  .matsum .chip{border:1px solid #333;border-radius:3px;padding:0 3px;font-size:10px;font-weight:bold;white-space:nowrap;}`;
const buildMatSummaryBody = (orders) => {
  const totals = materialTotals(orders);
  const count = orders.length;
  const t = new Date();
  const dateStr = `${String(t.getDate()).padStart(2,'0')}/${String(t.getMonth()+1).padStart(2,'0')}/${t.getFullYear()} ${String(t.getHours()).padStart(2,'0')}:${String(t.getMinutes()).padStart(2,'0')}`;
  const fmt = (n) => String(Math.round(n*10)/10).replace('.', ',');
  const chips = orders.map(o=>`<span class="chip">${o.orderNo}</span>`).join('');
  const row = (label, val, unit='') => val>0 ? `<tr><td>${label}</td><td style="text-align:right;font-weight:900">${fmt(val)}${unit?' '+unit:''}</td></tr>` : '';
  ...
  return `<div class="matsum" style="padding:10px;">
...
export default function CustomScreen({ customOrders, setCustomOrders, soldOrders, setSoldOrders, customers, onRequestAddCustomer, sasiStock={}, setSasiStock, caseStock={}, setCaseStock, sasiOrders=[], setSasiOrders, caseOrders=[], setCaseOrders, coatings=[], dipliSasiStock=[], setDipliSasiStock, locks=[], cylinders=[], misc=[], isGuest=false, locked=false, formOnly=false, forcedTab=null, setTabIndex, highlightOrderId = null, onClearSearchHighlight, currentUserName='', isAdmin=false, resolveName=(u)=>u, showCustomerLookup=false, setShowCustomerLookup=()=>{}, isSeller=false, sellerKey=null, filterSellerKey=null, editSubmission=null, onEditSubmissionDone=()=>{}, quotes=[], setQuotes=()=>{}, quotesOnly=false, isForeman=false }) {
  ...
  const openShipModal = (kind) => {
    const field = kind === 'ΣΥΣΚΕΥΑΣΙΑ' ? 'packaging' : 'agency';
    const list = (customOrders || []).filter(o => (o.status==='STD_BUILD'||o.status==='STD_READY'||o.status==='PROD') && !o.onHold && o[field]).sort((a,b)=>(parseInt(a.orderNo)||0)-(parseInt(b.orderNo)||0));
    if (!list.length) { setConfirmModal({ visible:true, title:kind, message:'Δεν υπάρχουν παραγγελίες.', confirmText:'ΟΚ', onConfirm:null }); return; }
    setShipSel(new Set(list.map((_,i)=>i)));
    setShipModal({ visible:true, kind, hits:list });
  };
  ...
  const [bfExcluded, setBfExcluded] = useState({});
  ...
  const linkActive = (src, l) => l === 'montage' ? src.installation === 'ΝΑΙ'
    ...
    : l === 'galva' ? (!!src.caseMaterial && src.caseMaterial !== 'DKP')
    : l === 'packaging' ? !!src.packaging
    : l === 'agency' ? !!src.agency : false;
  ...
  // Αφαιρεί αυτόματες χρεώσεις «δεμένων» ειδών (συσκευασία/πρακτορείο) όταν το αντίστοιχο κουμπί ξετσεκαριστεί.
  const dropInactiveLinks = (list, src) => (Array.isArray(list) ? list : []).filter(l => {
    const lab = String(l?.label || '').trim();
    const m = (misc || []).find(x => x && (x.link === 'packaging' || x.link === 'agency') && x.name === lab);
    return !m || linkActive(src, m.link);
  });
  // Συσκευασία & έξοδα πρακτορείου: πάντα στο τέλος της λίστας (άσχετα πότε μπήκαν).
  const tailLinks = (list) => {
    const arr = Array.isArray(list) ? list : [];
    const has = (link, l) => (misc || []).some(x => x && x.link === link && x.name === String(l?.label || '').trim());
    return [...arr.filter(l => !has('packaging', l) && !has('agency', l)), ...arr.filter(l => has('packaging', l)), ...arr.filter(l => has('agency', l))];
  };
  ...
  const openPriceModal = async (order) => {
    ...
    const merged = tailLinks(withTail(applyAutoPriceLines(dropInactiveLinks(dropStaleStav(dropStaleStavCol(src.priceList || []), stavRuleNames(cat)), src), await buildAutoLines(src, cat)), src));
    if (order) {
  ...
  // Εναλλαγή συσκευασίας/πρακτορείου από την κάρτα — ενημερώνει και τη χρέωση (μπαίνει/βγαίνει).
  const toggleOrderShip = async (order, field) => {
    const on = !order[field];
    const names = (misc || []).filter(x => x && x.link === field).map(x => x.name);
    let pl = (order.priceList || []).filter(l => !names.includes(String(l?.label || '').trim()));
    if (on) {
      const doors = parseInt(order.qty, 10) > 0 ? parseInt(order.qty, 10) : 1;
      for (const it of (misc || []).filter(x => x && x.link === field)) {
        const price = parseFloat(String(it.price).replace(',', '.'));
        if (!isNaN(price) && price > 0) pl.push({ label: it.name, value: String(it.price), qty: String(doors), def: String(it.price) });
      }
    }
    pl = tailLinks(pl);
    const priceTotal = priceFinalTotal(pl, order.priceDiscount);
    const priceLog = appendPriceLog(order.priceLog, priceTotal, pl.length > 0);
    const upd = { ...order, [field]: on, priceList: pl, priceTotal, priceLog };
    setCustomOrders(prev => prev.map(o => o.id === order.id ? upd : o));
    setSoldOrders(prev => prev.map(o => o.id === order.id ? upd : o));
    try { await fetch(`${FIREBASE_URL}/std_orders/${order.id}.json`, { method: 'PATCH', body: JSON.stringify({ [field]: on, priceList: pl, priceTotal, priceLog }) }); } catch {}
  };
  const renderShipBoxes = (order, ro = false) => {
    const mk = (field, label) => {
      const on = !!order[field];
      const st = { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: on ? '#e8f5e9' : '#f0f0f0', borderRadius: 6, paddingVertical: 6, paddingHorizontal: 8, borderWidth: 1.5, borderColor: on ? '#00C851' : '#ccc' };
      const inner = (<><Text style={{ fontSize: 12 }}>{on ? '✅' : '⬜'}</Text><Text style={{ fontSize: 11, fontWeight: 'bold', color: on ? '#2e7d32' : '#9e9e9e' }}>{label}</Text></>);
      return ro
        ? <View key={field} style={st}>{inner}</View>
        : <TouchableOpacity key={field} onPress={() => setConfirmModal({ visible: true, title: 'Επιβεβαίωση', message: `Αλλαγή «${label}» σε ${on ? 'ΟΧΙ' : 'ΝΑΙ'};`, confirmText: 'ΝΑΙ, ΑΛΛΑΓΗ', onConfirm: () => toggleOrderShip(order, field) })} style={st}>{inner}</TouchableOpacity>;
    };
    return <View style={{ flexDirection: 'row', gap: 6 }}>{mk('packaging', 'Συσκευασία')}{mk('agency', 'Πρακτορείο')}</View>;
  };
  const renderDocButton = (order) => {
    ...
    return (
      <View style={{flexDirection:'row', alignItems:'flex-start', flexWrap:'wrap', gap:8, marginTop:8}}>
        ...
        {renderShipBoxes(order, locked || isSeller || isForeman)}
      </View>
    );
  };
  ...
  // Φάση 2: πελάτης «πρακτορείο» → αυτόματο τσεκ σε Συσκευασία + Πρακτορείο (μία φορά ανά επιλογή· ξετσεκάρεται χειροκίνητα).
  const agencyAutoRef = useRef(null);
  useEffect(() => {
    if (editingOrder || editingQuote) return;
    const c = selectedCustomer;
    if (!c) { agencyAutoRef.current = null; return; }
    if (c.agency && agencyAutoRef.current !== c.id) {
      agencyAutoRef.current = c.id;
      setCustomForm(f => ({ ...f, packaging: true, agency: true }));
    }
  }, [selectedCustomer, editingOrder, editingQuote]);
  ...
  const splitOrder = async (order, peelQty) => {
    ...
    // Ρίζα αρίθμησης: σπάσιμο σπασμένου κομματιού αριθμείται σαν να σπάει η μαμά (ίδια ρίζα → επόμενο αδερφάκι).
    const root = order.splitChild ? parentNo.replace(/-\d+$/, '') : (parentNo.includes('-') ? parentNo : `${parentNo}-0`);
    const newNo = nextSiblingNo(root);
    ...
    const remaining = order.splitChild
      ? {...order, orderNo: motherNo, qty:String(totalQty-qty), groupId:gId}
      : {...order, orderNo: motherNo, qty:String(totalQty-qty), groupId:gId, origQty: motherOrig};
  ...
  const renderQtyBox = (o) => {
    ...
    const canSplit = (isAdmin || (isForeman && !locked)) && q>1;
    return (
  ...
  const renderBuildFilterPanel = () => {
    ...
    const finalMatches = matches.filter(o => !bfExcluded[o.id]);
    const allSel = matches.length>0 && matches.every(o => !bfExcluded[o.id]);
    const toggle = (task, state) => { setBfExcluded({}); setBuildFilterSel(prev => {
      ...
      <View style={{position:'absolute', top: bfTop, right: bfRight, width:380, backgroundColor:'#fff8f2', borderWidth:1, borderColor:'#e65100', borderRadius:8, zIndex:2000, elevation:24, shadowColor:'#000', shadowOffset:{width:0,height:6}, shadowOpacity:0.3, shadowRadius:12}}>
        ...
        <View style={{padding:10}}>
          ...
          <View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between',marginTop:4,borderTopWidth:1,borderTopColor:'#f0d9c6',paddingTop:8}}>
            <Text style={{fontWeight:'bold',color:'#e65100',fontSize:13}}>Επιλεγμένες: {finalMatches.length}/{matches.length}</Text>
            <TouchableOpacity disabled={!finalMatches.length}
              style={{backgroundColor: finalMatches.length?'#e65100':'#ccc',paddingHorizontal:12,paddingVertical:6,borderRadius:6}}
              onPress={()=>{ const c=buildFilterTitle(); handleSelectionPrint(finalMatches, c?`${title} — ${c}`:title); }}>
          ...
          {matches.length>0&&(<>
            <TouchableOpacity style={{flexDirection:'row',alignItems:'center',gap:6,marginTop:8}}
              onPress={()=>setBfExcluded(allSel ? Object.fromEntries(matches.map(o=>[o.id,true])) : {})}>
              <View style={{width:16,height:16,borderRadius:4,borderWidth:2,borderColor:allSel?'#00C851':'#bbb',backgroundColor:allSel?'#00C851':'white',alignItems:'center',justifyContent:'center'}}>
                {allSel&&<Text style={{color:'white',fontWeight:'bold',fontSize:10}}>✓</Text>}
              </View>
              <Text style={{fontSize:12,fontWeight:'bold',color:'#333'}}>Επιλογή όλων</Text>
            </TouchableOpacity>
            <ScrollView style={{maxHeight:220,marginTop:6}}>
              {matches.map(o=>{
                const sel=!bfExcluded[o.id];
                return (
                  <TouchableOpacity key={o.id} style={{flexDirection:'row',alignItems:'center',gap:6,paddingVertical:3}}
                    onPress={()=>setBfExcluded(p=>{ const n={...p}; if(n[o.id]) delete n[o.id]; else n[o.id]=true; return n; })}>
                    <View style={{width:16,height:16,borderRadius:4,borderWidth:2,borderColor:sel?'#00C851':'#bbb',backgroundColor:sel?'#00C851':'white',alignItems:'center',justifyContent:'center'}}>
                      {sel&&<Text style={{color:'white',fontWeight:'bold',fontSize:10}}>✓</Text>}
                    </View>
  ...
  // Πίνακας εκτύπωσης επενδύσεων — ενότητες ΠΑΡΑΓΓΕΛΙΕΣ + ΠΡΟΣ ΚΑΤΑΣΚΕΥΗ (μονή)· μόνο ΠΡΟΣ ΚΑΤΑΣΚΕΥΗ (διπλή)
  const renderCoatPrintPanel = () => {
    const isDipli = forcedTab==='ΔΙΠΛΗ';
    const withCoats = (list) => list.filter(o => stdCoatNames(o).length>0);
    const groups = isDipli
      ? [{title:'ΠΡΟΣ ΚΑΤΑΣΚΕΥΗ', orders: withCoats(stdBuildDipliOrders)}]
      : [{title:'ΠΑΡΑΓΓΕΛΙΕΣ', orders: withCoats(moniOrders)}, {title:'ΠΡΟΣ ΚΑΤΑΣΚΕΥΗ', orders: withCoats(stdBuildMoniOrders)}];
    const coatOrders = groups.flatMap(g=>g.orders);
        ...
        <View style={{padding:10}}>
          {coatOrders.length===0 ? (
            ...
            <ScrollView style={{maxHeight:280}}>
              {groups.map(g => g.orders.length>0 && (
                <View key={g.title} style={{marginBottom:6}}>
                  <Text style={{fontSize:12, fontWeight:'900', color:'#3949ab', backgroundColor:'#e8eaf6', paddingHorizontal:6, paddingVertical:3, borderRadius:4, marginBottom:2}}>{g.title} ({g.orders.length})</Text>
                  {g.orders.map(orderRow)}
                </View>
              ))}
            </ScrollView>
            <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginTop:8, borderTopWidth:1, borderTopColor:'#d7d9ef', paddingTop:8}}>
              ...
              <TouchableOpacity disabled={!selected.length}
                ...
                onPress={async()=>{ const list=[...selected].sort((a,b)=>(parseInt(a.orderNo)||0)-(parseInt(b.orderNo)||0)); await printHTML(buildEpendStdHtml(list, list), 'VAICON — ΕΚΤΥΠΩΣΗ ΠΡΟΣ ΠΑΡΑΓΩΓΗ'); }}>
  ...
  const handleMaterialsPrint = (orders) => {
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      body{margin:0;} ${MAT_SUMMARY_CSS}
      @media print{@page{size:A4 portrait;margin:10mm;}*{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}
    </style></head><body>${buildMatSummaryBody(orders)}</body></html>`;
    if (Platform.OS==='web') {
      const win = window.open('','_blank');
      if (!win) return Alert.alert('Σφάλμα','Επιτρέψτε τα pop-ups.');
  ...
  const renderMaterialsPanel = () => {
    ...
    const bodyMaxH = Math.min((Platform.OS==='web' ? (window.innerHeight||800)/matZoom() : 800) - 240, 400);
    const header = (
    ...
    const panel = (
      <View style={[{position:'absolute', ...posStyle, width:420, backgroundColor:'#f1f8f1', borderWidth:1, borderColor:'#2e7d32', borderRadius:8, zIndex:99999, elevation:24, shadowColor:'#000', shadowOffset:{width:0,height:6}, shadowOpacity:0.3, shadowRadius:12}, Platform.OS==='web'?{position:'fixed'}:{}]}>
        {Platform.OS==='web'
          ...
          {materialsPool.length===0 ? (
            ...
            <View style={{flexDirection:'row', alignItems:'center', justifyContent:'flex-end', marginTop:8, borderTopWidth:1, borderTopColor:'#c8e6c9', paddingTop:8}}>
              <TouchableOpacity disabled={!selected.length}
                ...
                onPress={()=>handleMaterialsPrint(selected)}>
                <Text style={{color:'white', fontWeight:'bold', fontSize:12}}>🖨️ ΕΚΤΥΠΩΣΗ</Text>
              </TouchableOpacity>
            </View>
          </>)}
    ...
    if (Platform.OS==='web') { try { return require('react-dom').createPortal(panel, document.body); } catch { return panel; } }
    return panel;
  };
  ...
  // Χαρτί ΣΥΣΚΕΥΑΣΙΑ / ΠΡΑΚΤΟΡΕΙΟ — μπλοκ ανά παραγγελία (ίδιο με τις ειδικές).
  const buildShipPrintHTML = (orders, kind) => {
    const esc = (s) => String(s==null?'':s).replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));
    const today = new Date();
    const dateStr = `${String(today.getDate()).padStart(2,'0')}/${String(today.getMonth()+1).padStart(2,'0')}/${today.getFullYear()}`;
    const slips = [...orders].sort((a,b)=>(parseInt(a.orderNo)||0)-(parseInt(b.orderNo)||0)).map(o => {
      const c = findCustomerOf(o) || {};
      const phone = [c.phone, c.phone2, c.phone3].filter(Boolean).join('  ·  ');
      const addr = c.city || '';
      const base = o.orderNo || '—';
      const det = [];
      const add = (l,v) => { const t=v==null?'':String(v).trim(); if(t) det.push(`<span class="lbl">${l}:</span> <b>${esc(t)}</b>`); };
      add('Διάσταση', `${o.h||'—'}×${o.w||'—'}`);
      const qtyBig = parseInt(o.qty,10)>1 ? `<div class="qty">${esc(o.qty)} ΤΕΜΑΧΙΑ</div>` : '';
      const stav = (o.stavera||[]).filter(s=>s&&s.dim).map(s=>(s.qty?`${s.qty}τεμ `:'')+[s.dim,s.design,s.color].filter(Boolean).join(' ')).join(' | ');
      ...
      add('Φορά', o.side==='ΑΡΙΣΤΕΡΗ'?'ΑΡΙΣΤΕΡΗ':'ΔΕΞΙΑ');
      add('Θωράκιση', (o.sasiType||'').includes('ΔΙΠΛΗ')?'ΔΙΠΛΗ':'ΜΟΝΗ');
      if (o.kypri==='ΝΑΙ') add('Κυπρί','ΝΑΙ');
      const glass = [o.glassDim,o.glassNotes].filter(Boolean).join(' ');
      if (glass) add('Τζάμι', glass);
      if (o.hardware) add('Χρ. Εξαρτ.', o.hardware);
      const coat = (o.coatings||[]).filter(n=>n&&String(n).trim()).join(', ');
      if (coat) add('Επένδυση', coat);
      const notes = o.notes!=null?String(o.notes).trim():'';
      return `<article class="slip">
        <div class="head">
          <div class="l1"><span class="no">${esc(base)}</span> <span class="cust">${esc(o.customer||c.name||'—')}</span></div>
          ${phone?`<div class="ph">📞 ${esc(phone)}</div>`:''}
          ${addr?`<div class="addr">📍 ${esc(addr)}</div>`:''}
        </div>
        ${qtyBig}
        ${det.length?`<div class="det">${det.join(' · ')}</div>`:''}
        ${notes?`<div class="notes"><b>Παρατηρήσεις:</b> ${esc(notes)}</div>`:''}
        ${kind==='ΠΡΑΚΤΟΡΕΙΟ'?'':`<div class="phase"><span class="lbl">Φάση:</span> <b>${esc(getOrderTabInfo(o).label)}</b></div>`}
      </article>`;
    }).join('');
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>VAICON — ${esc(kind)}</title><style>
      body{font-family:Arial,Helvetica,sans-serif;margin:8mm;color:#111;font-size:12px;}
      h1{font-size:16px;margin:0 0 8px 0;}
      .slip{page-break-inside:avoid;padding:6px 0;border-bottom:1px dashed #999;}
      .slip:last-child{border-bottom:none;}
      .head{margin-bottom:2px;}
      .head .l1{display:flex;flex-wrap:wrap;align-items:baseline;gap:14px;}
      .head .no{font-size:38px;font-weight:900;letter-spacing:1px;}
      .head .cust{font-size:30px;font-weight:bold;}
      .head .ph{font-size:28px;color:#0d47a1;font-weight:900;margin-top:2px;}
      .head .addr{font-size:22px;color:#222;font-weight:bold;margin-top:2px;}
      .qty{font-size:24px;font-weight:900;color:#c62828;margin-top:3px;}
      .det{margin-top:3px;font-size:12px;line-height:1.4;}
      .det .lbl{color:#555;}
      .notes{margin-top:3px;font-size:12px;white-space:pre-wrap;}
      .phase{margin-top:3px;font-size:12px;color:#1a1a1a;}
      @media print{@page{size:A4;margin:8mm;}}
    </style></head><body>
      <h1>🖨️ ${esc(kind)} — ${dateStr} (${orders.length})</h1>
      ${slips}
    </body></html>`;
  };
  const handleShipPrint = async (orders, kind) => {
    if (!orders.length) return;
    try { await printHTML(buildShipPrintHTML(orders, kind), `VAICON — ${kind}`); }
  ...
  // Ενεργές παραγγελίες για «Σύνολο Υλικών» (όλες ΜΟΝΗ+ΔΙΠΛΗ, όχι έτοιμες/πουλημένες/αναμονή/προσφορές)
  const materialsPool = useMemo(() => baseCustom.filter(o=>o.orderType==='ΤΥΠΟΠΟΙΗΜΕΝΗ'&&o.status!=='STD_READY'&&o.status!=='STD_SOLD'&&o.status!=='QUOTE'&&!o.isQuote&&orderHasMaterials(o)).sort((a,b)=>(parseInt(a.orderNo)||0)-(parseInt(b.orderNo)||0)), [baseCustom]);
  ...
  return (
    <View style={{flex:1, flexDirection:'row'}}>
        ...
        <View style={[{position:'absolute', top:0, left:0, right:0, bottom:0, backgroundColor:'rgba(0,0,0,0.5)', justifyContent:'center', alignItems:'center', zIndex:2000}, Platform.OS==='web'?{position:'fixed'}:{}]}>
          <View style={{backgroundColor:'#fff', borderRadius:14, padding:16, width:'100%', maxWidth:760, maxHeight:'88%'}}>
            <View style={{flexDirection:'row', alignItems:'center', justifyContent:'space-between', marginBottom:8}}>
              <Text style={{fontSize:17, fontWeight:'bold', color:'#1a1a1a'}}>🖨️ {shipModal.kind} ({shipModal.hits.length})</Text>
            </View>
            <View style={{flexDirection:'row', alignItems:'center', gap:8, marginBottom:8, flexWrap:'wrap'}}>
              <TouchableOpacity onPress={()=>setShipSel(new Set(shipModal.hits.map((_,i)=>i)))} style={{backgroundColor:'#eee', borderRadius:8, paddingHorizontal:10, paddingVertical:6}}><Text style={{fontWeight:'bold', color:'#555', fontSize:13}}>Όλα ✓</Text></TouchableOpacity>
              <TouchableOpacity onPress={()=>setShipSel(new Set())} style={{backgroundColor:'#eee', borderRadius:8, paddingHorizontal:10, paddingVertical:6}}><Text style={{fontWeight:'bold', color:'#555', fontSize:13}}>Καθάρισμα</Text></TouchableOpacity>
              <TouchableOpacity disabled={shipSel.size===0} onPress={()=>{ const orders=[...shipSel].sort((a,b)=>a-b).map(i=>shipModal.hits[i]).filter(Boolean); if(orders.length){ handleShipPrint(orders, shipModal.kind); setShipModal({visible:false,kind:'',hits:[]}); } }} style={{backgroundColor: shipSel.size?'#1565C0':'#ccc', borderRadius:8, paddingHorizontal:10, paddingVertical:6, marginLeft:'auto'}}><Text style={{fontWeight:'bold', color:'#fff', fontSize:13}}>Εκτύπωση ({shipSel.size}) 🖨️</Text></TouchableOpacity>
            </View>
            <ScrollView style={{maxHeight:420}}>
              {shipModal.hits.map((o,i)=>{
                const tab=getOrderTabInfo(o); const isSel=shipSel.has(i);
                const tint=o.status==='STD_READY'?'#e8f5e9':'#fff3e0';
                const coat=(o.coatings||[]).filter(n=>n&&String(n).trim()).join(', ');
                const c=findCustomerOf(o)||{}; const phones=[c.phone,c.phone2,c.phone3].filter(Boolean); const addr=c.city||'';
                return (
                  <View key={o.id+'-'+i} style={{flexDirection:'row', alignItems:'flex-start', gap:8, padding:8, borderBottomWidth:1, borderBottomColor:'#eee', backgroundColor:tint}}>
                    <TouchableOpacity onPress={()=>setShipSel(p=>{const n=new Set(p); n.has(i)?n.delete(i):n.add(i); return n;})} style={{width:26, alignItems:'center', paddingTop:2}}><Text style={{fontSize:20, color:'#1565C0'}}>{isSel?'☑':'☐'}</Text></TouchableOpacity>
                    <View style={{flex:1}}>
                      <View style={{flexDirection:'row', alignItems:'center', gap:8, flexWrap:'wrap'}}>
                        <Text style={{fontSize:16, fontWeight:'900', color:'#1a1a1a'}}>{o.orderNo||'—'}</Text>
                        <Text style={{fontSize:14, fontWeight:'bold', color:'#333'}}>{o.customer||'—'}</Text>
                        {addr?<Text style={{fontSize:12, color:'#333'}}>📍 {addr}</Text>:null}
                        <View style={{backgroundColor:tab.color, borderRadius:4, paddingHorizontal:6, paddingVertical:1, marginLeft:'auto'}}><Text style={{color:'#fff', fontWeight:'bold', fontSize:10}}>{tab.label}</Text></View>
                      </View>
                      {phones.length?<Text style={{fontSize:18, color:'#0d47a1', fontWeight:'900', marginTop:2}}>📞 {phones.join('   ·   ')}</Text>:null}
                      <View style={{flexDirection:'row', alignItems:'center', gap:6, flexWrap:'wrap', marginTop:2}}>
                        <Text style={{fontSize:13, color:'#1a1a1a', fontWeight:'bold'}}>{o.h||'—'}×{o.w||'—'}</Text>
                        <Text style={{fontSize:12, color:'#555'}}>{o.side==='ΑΡΙΣΤΕΡΗ'?'ΑΡ':'ΔΕ'}</Text>
                        <Text style={{fontSize:12, color:'#555'}}>{(o.sasiType||'').includes('ΔΙΠΛΗ')?'Δ/Θ':'Μ/Θ'}</Text>
                        {parseInt(o.qty,10)>1 ? <Text style={{fontSize:12, color:'#cc0000', fontWeight:'900'}}>{o.qty}τεμ</Text> : null}
                        {o.kypri==='ΝΑΙ'?<Text style={{fontSize:11, color:'#6a1b9a', fontWeight:'bold'}}>ΚΥΠΡΙ</Text>:null}
                        {coat?<Text style={{fontSize:11, color:'#b8860b', fontWeight:'bold'}} numberOfLines={1}>🎨 {coat}</Text>:null}
                      </View>
                    </View>
                  </View>
                );
            ...
            <TouchableOpacity onPress={()=>setShipModal({visible:false,kind:'',hits:[]})} style={{backgroundColor:'#1a1a1a', borderRadius:10, padding:12, alignItems:'center', marginTop:10}}>
      ...
      >
        <View style={{paddingBottom:120}}>
          ...
          {!isSeller && <TextInput ref={orderNoRef} style={[styles.input, {fontSize:18, fontWeight:'bold', width:90, letterSpacing:1, marginBottom:0}]} placeholder="Ν/Π" keyboardType="numeric" value={customForm.orderNo} selectTextOnFocus
            ...
            <View style={{flexDirection:'row', gap:6, alignItems:'flex-end', marginLeft:48}}>
              <TouchableOpacity onPress={()=>setCustomForm(f=>{ const on=!f.packaging; let pl=f.priceList||[]; if(!on){ const names=(misc||[]).filter(x=>x&&x.link==='packaging').map(x=>x.name); pl=pl.filter(l=>!names.includes(String(l?.label||'').trim())); } return {...f, packaging:on, priceList:pl}; })}
                style={{flexDirection:'row', alignItems:'center', justifyContent:'center', gap:5, height:48, backgroundColor: customForm.packaging?'#e8f5e9':'#f0f0f0', borderRadius:8, paddingHorizontal:10, borderWidth:2, borderColor: customForm.packaging?'#00C851':'#cccccc'}}>
                <Text style={{fontSize:15}}>{customForm.packaging?'✅':'⬜'}</Text>
                <Text style={{fontSize:13, fontWeight:'bold', color: customForm.packaging?'#2e7d32':'#9e9e9e'}}>Συσκευασία</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={()=>setCustomForm(f=>{ const on=!f.agency; let pl=f.priceList||[]; if(!on){ const names=(misc||[]).filter(x=>x&&x.link==='agency').map(x=>x.name); pl=pl.filter(l=>!names.includes(String(l?.label||'').trim())); } return {...f, agency:on, priceList:pl}; })}
                style={{flexDirection:'row', alignItems:'center', justifyContent:'center', gap:5, height:48, backgroundColor: customForm.agency?'#e8f5e9':'#f0f0f0', borderRadius:8, paddingHorizontal:12, borderWidth:2, borderColor: customForm.agency?'#00C851':'#cccccc'}}>
                <Text style={{fontSize:15}}>{customForm.agency?'✅':'⬜'}</Text>
                <Text style={{fontSize:13, fontWeight:'bold', color: customForm.agency?'#2e7d32':'#9e9e9e'}}>Πρακτορείο</Text>
              </TouchableOpacity>
            </View>
                    ...
                    <>
                      <TouchableOpacity
                        ...
                        {!isSeller && <View style={{flexDirection:'row', gap:6, alignItems:'center'}}>
                          ...
                          <TouchableOpacity
                            style={{backgroundColor:'white', paddingHorizontal:8, paddingVertical:4, borderRadius:6}}
                            onPress={e=>{e.stopPropagation?.(); setBuildFilterOpen(false); setCoatPrintOpen(v=>!v);}}>
                            <Text style={{color:'#5c6bc0', fontSize:10, fontWeight:'bold'}}>🎨 ΕΠΕΝΔΥΣΕΙΣ</Text>
                          </TouchableOpacity>
                          <TouchableOpacity
                            style={{backgroundColor:'white', paddingHorizontal:8, paddingVertical:4, borderRadius:6}}
      ...
      {!formOnly && coatPrintOpen && !isSeller && (showSec('orders') || (forcedTab==='ΔΙΠΛΗ' && showSec('build'))) && renderCoatPrintPanel()}
      {!formOnly && placePrintOpen && !isSeller && showSec('orders') && renderPlacePrintPanel()}
      {!formOnly && materialsOpen && !isSeller && !isForeman && !isGuest && renderMaterialsPanel()}
      </View>
        ...
        <View style={{width:240, backgroundColor:'#f7f7f7', borderLeftWidth:1, borderLeftColor:'#e0e0e0'}}>
          ...
          {!isSeller && (
            <View style={{marginTop:12, flexDirection:'row', gap:6}}>
              <TouchableOpacity style={{flex:1, backgroundColor:'#00838f', borderRadius:8, paddingVertical:8, paddingHorizontal:4, minHeight:40, alignItems:'center', justifyContent:'center'}} onPress={()=>openShipModal('ΣΥΣΚΕΥΑΣΙΑ')}>
                <Text style={{color:'white', fontWeight:'bold', fontSize:11, textAlign:'center'}}>🖨️ ΣΥΣΚΕΥΑΣΙΑ</Text>
              </TouchableOpacity>
              <TouchableOpacity style={{flex:1, backgroundColor:'#5d4037', borderRadius:8, paddingVertical:8, paddingHorizontal:4, minHeight:40, alignItems:'center', justifyContent:'center'}} onPress={()=>openShipModal('ΠΡΑΚΤΟΡΕΙΟ')}>
                <Text style={{color:'white', fontWeight:'bold', fontSize:11, textAlign:'center'}}>🖨️ ΠΡΑΚΤΟΡΕΙΟ</Text>
              </TouchableOpacity>
            </View>
          )}
          {!isSeller && !isForeman && !isGuest && (
            <View style={{marginTop:6, flexDirection:'row', gap:6}}>
              <TouchableOpacity style={{flex:1, backgroundColor: materialsOpen?'#1b5e20':'#2e7d32', borderRadius:8, paddingVertical:8, paddingHorizontal:4, minHeight:40, alignItems:'center', justifyContent:'center'}} onPress={()=>setMaterialsOpen(v=>!v)}>
