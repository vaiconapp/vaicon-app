...
import { StyleSheet, Text, View, FlatList, TextInput, TouchableOpacity, Alert, Platform } from 'react-native';
...
export default function MessagesScreen({ users = [], userLabels = {}, lockKey, onClose }) {
  ...
  const loadArchive = useCallback(async (silent = false) => {
    ...
    try {
      const r = await fetch(`${FIREBASE_URL}/messages.json`);
  ...
  const send = async () => {
    ...
    for (const u of targets) {
      ...
      try {
        const r = await fetch(`${FIREBASE_URL}/messages/${lockKey(u)}.json`, { method: 'POST', body: JSON.stringify(entry) });
        if (!r.ok) failed++;
      } catch { failed++; }
    }
    setSending(false);
  ...
  const deleteEntry = (item) => {
    const doDel = async () => {
      try { await fetch(`${FIREBASE_URL}/messages/${item.uk}/${item.id}.json`, { method: 'DELETE' }); setArchive(a => a.filter(x => x.id !== item.id)); } catch {}
    };
...
const styles = StyleSheet.create({
  ...
  header: { backgroundColor: '#1a1a2e', padding: 16, paddingTop: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  closeBtn: { padding: 4 },
