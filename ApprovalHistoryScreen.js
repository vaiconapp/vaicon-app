...
// Ιστορικό εγκρίσεων (users + admin). Αυτόματη διαγραφή >4 μηνών.
export default function ApprovalHistoryScreen({ onClose, resolveLabel = (u) => u }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      // Αυτόματο καθάρισμα παλαιών (>4 μηνών)
      const old = {};
              ...
              <View style={styles.modeDropdown}>
                {Object.keys(MODES).map(k => (
                  <TouchableOpacity key={k} style={styles.modeItem} onPress={() => { setMode(k); setModeOpen(false); setSearch(''); }}>
                    <Text style={[styles.modeItemText, mode === k && { fontWeight: 'bold', color: '#1565C0' }]}>{MODES[k]}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  modeBtn: { backgroundColor: '#1565C0', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 9 },
  ...
  searchBox: { flexDirection: 'row', alignItems: 'center', width: '33%', minWidth: 150, backgroundColor: '#E3F2FD', borderRadius: 8, borderWidth: 1.5, borderColor: '#64B5F6', paddingHorizontal: 8 },
  searchInput: { flex: 1, paddingVertical: 8, fontSize: 14, color: '#1a1a1a' },
  clearX: { fontSize: 16, color: '#1565C0', fontWeight: 'bold', paddingHorizontal: 4 },
