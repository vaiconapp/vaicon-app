...
group('materialTotals — Σύνολο Υλικών', () => {
  const t1 = materialTotals([{ qty:'2', coatings:['ΛΑΜΙΝΕΪΤ ΚΑΡΥΔΙΑ ΕΞΩ'], caseType:'ΚΛΕΙΣΤΟΥ ΤΥΠΟΥ' }]);
  test('2 πόρτες laminate εξωτ → 2 φύλλα', t1.coatings, [{ label:'ΛΑΜΙΝΕΙΤ ΚΑΡΥΔΙΑ', qty:2 }]);
  test('2 πόρτες → κάσα κλειστή 5', t1.caseClosed, 5);
  test('2 πόρτες → προφίλ εξωτ 5', t1.profExo, 5);
  test('χωρίς μέσα → προφίλ εσωτ 0', t1.profMesa, 0);
  test('χωρίς πηχάκι → 0', t1.pihaki, 0);

  const t2 = materialTotals([{ qty:'1', coatings:['Λαμινέιτ Καρυδιά ΕΞΩ','ΛΑΜΙΝΕΪΤ ΚΑΡΥΔΙΑ ΜΕΣΑ'], caseType:'ΚΛΕΙΣΤΟΥ ΤΥΠΟΥ' }]);
  test('έξω+μέσα ίδιο χρώμα → 2 φύλλα ίδιο κλειδί', t2.coatings, [{ label:'ΛΑΜΙΝΕΙΤ ΚΑΡΥΔΙΑ', qty:2 }]);
  test('προφίλ εξωτ 2,5', t2.profExo, 2.5);
  test('προφίλ εσωτ 2,5', t2.profMesa, 2.5);

  const t3 = materialTotals([{ qty:'2', coatings:['RAL ΜΕΣΑ'], caseType:'ΑΝΟΙΧΤΟΥ ΤΥΠΟΥ', coatingDetails:{ 'RAL ΜΕΣΑ':{ pihaki:true } } }]);
  test('κάσα ανοιχτή 5', t3.caseOpen, 5);
  test('πηχάκι τσεκαρισμένο → 5', t3.pihaki, 5);
  test('προφίλ εσωτ 5', t3.profMesa, 5);

  test('χωρίς επένδυση → καμία επένδυση', materialTotals([{ qty:'1', caseType:'ΚΛΕΙΣΤΟΥ ΤΥΠΟΥ' }]).coatings, []);
  test('χωρίς qty → 1 πόρτα', materialTotals([{ coatings:['RAL ΕΞΩ'] }]).profExo, 2.5);
});
