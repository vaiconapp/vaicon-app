...
export default function SellerSubmissionsScreen({ sellerKey, onEditSubmission, coatings = [] }) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const mine = list.filter(s => s.submittedBy === sellerKey && s.orderType === 'ΤΥΠΟΠΟΙΗΜΕΝΗ');
  ...
  const renderCard = (sub, kind, inGroup) => {
    ...
    <View key={sub._sid} style={[styles.card, { borderLeftColor: isRejected ? '#c62828' : isApproved ? '#2e7d32' : '#ff9800' }]}>
      <View style={{ flex: 1 }}>
        ...
        <StdOrderPreview order={sub} coatings={coatings} showCustomer={!inGroup} />
...
const styles = StyleSheet.create({
  ...
  quoteTag: { alignSelf: 'flex-start', fontSize: 12, color: '#6a1b9a', fontWeight: 'bold', backgroundColor: '#f3e5f5', borderColor: '#8e24aa', borderWidth: 1, borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2, marginBottom: 4 },
