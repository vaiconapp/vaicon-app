...
// βρίσκει το mapping στο /msg_map/{id} και ενημερώνει το std_orders/{id}/msgStatus/{channel}.
// Env: FIREBASE_DB_URL
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const cur = await (await fbFetch(`${db}/std_orders/${map.orderId}/msgStatus/${channel}.json`)).json().catch(() => null);
    const curRank = STATUS_RANK[cur?.status] || 0;
    const newRank = STATUS_RANK[status] || 0;
    if (status !== 'failed' && newRank < curRank) return ok('stale');

    await fbFetch(`${db}/std_orders/${map.orderId}/msgStatus/${channel}.json`, {
      method: 'PUT',
