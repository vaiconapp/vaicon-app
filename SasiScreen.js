...
import { sasiKey as stockKey, resDeferred } from './stockUtils';
...
export default function SasiScreen({ sasiStock={}, setSasiStock, opsBasket=[], setOpsBasket, stockHighlight=null, onClearSearchHighlight, locked=false, isAdmin=false, isForeman=false, customOrders=[] }) {
  ...
  const handleSubtract = (key, label, stockMax, pending) => {
    if (locked) return;
    if (stockMax <= 0 && pending <= 0 && !isAdmin) return Alert.alert('Προσοχή','Δεν υπάρχει ποσότητα για αφαίρεση.');
  ...
  const handleQtyConfirm = (n) => {
    ...
    if (mode === 'sub' && n > total && !isAdmin) return Alert.alert('Προσοχή',`Μπορείτε να αφαιρέσετε έως ${total} τεμάχια.`);
    setOpsBasket(prev => [...prev, { id: Date.now()+'_'+Math.random(), key, mode, n, dim: dim || key.replace(/_/g,'x'), allowNeg: isAdmin && mode==='sub' }]);
  };
  ...
  const applyOldCover = async (key) => {
    ...
    setSasiStock(prev => ({ ...prev, [key]: upd }));
    setOldConfirm(false); setOldSel([]);
    await syncKey(key, upd);
    for (const no of oldSel) await logActivity('ΣΑΣΙ ΣΤΟΚ', 'Κάλυψη από παλιό στοκ', { size: key.replace(/_/g,'x'), qty: '#'+no });
  };
  ...
  const undoOldCover = (key, orderNo) => {
    const doIt = async () => {
      ...
      setSasiStock(prev => ({ ...prev, [key]: upd }));
      await syncKey(key, upd);
      await logActivity('ΣΑΣΙ ΣΤΟΚ', 'Αναίρεση κάλυψης παλιού στοκ', { size: key.replace(/_/g,'x'), qty: '#'+orderNo });
    };
            ...
            <View key={key} style={[styles.tableRow, available<0&&{backgroundColor:'#fff5f5'}, rowHL && { backgroundColor: '#fff8e1', borderWidth: 2, borderColor: '#FFC107' }, pendingKeys.has(key) && { backgroundColor:'#e3f2fd', borderWidth:2, borderColor:'#1976d2' }]}>
              ...
              <View style={[styles.tdWrap, {width:34, justifyContent:'center', alignItems:'center'}]}>
                {!locked&&<TouchableOpacity style={[styles.subBtn, ((entry.qty||0)<=0&&pending<=0)&&{opacity:0.35}]}
                  disabled={!isAdmin&&(entry.qty||0)<=0&&pending<=0}
  ...
  const handlePrintProd = () => {
    ...
    const buildSide = (side) => {
      ...
      const rows = HEIGHTS.flatMap(h => WIDTHS.map(w => {
        const pending = (stockMap[stockKey(h, w, side)] || {}).pending || 0;
