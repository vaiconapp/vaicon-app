...
export default function SasiScreen({ sasiOrders, setSasiOrders, soldSasiOrders, setSoldSasiOrders }) {
  ...
  const updateStatus = async (id, newStatus) => {
    ...
    if (newStatus === 'SOLD') {
      ...
      if (totalQty <= 1) {
        ...
        setSasiOrders(sasiOrders.filter(o => o.id !== id));
  ...
  const handleSellConfirm = async (sellQty) => {
    ...
    if (sellQty === totalQty) {
      ...
      setSoldSasiOrders([soldEntry, ...soldSasiOrders]);
  ...
  const cancelOrder = (id) => Alert.alert("Ακύρωση", "Οριστική διαγραφή;", [{ text: "Όχι" }, { text: "Ναι", style: "destructive", onPress: async () => { setSasiOrders(sasiOrders.filter(o => o.id !== id)); await deleteFromCloud(id); } }]);
  ...
  return (
    ...
    <ScrollView style={{ padding: 10 }}>
      <View style={{ paddingBottom: 120 }}>
        <View style={styles.bigHeader}><Text style={styles.bigHeaderTxt}>🚪 ΤΥΠΟΠΟΙΗΜΕΝΑ ΣΑΣΙ</Text></View>
        <Text style={styles.sectionTitle}>ΚΑΤΑΧΩΡΗΣΗ ΤΥΠΟΠΟΙΗΜΕΝΟΥ ΣΑΣΙ</Text>

        <Text style={styles.label}>Θωράκιση:</Text>
        ...
        {[['pending','ΠΡΟΣ ΠΑΡΑΓΩΓΗ','#ff4444'],['prod','ΣΤΗΝ ΠΑΡΑΓΩΓΗ','#ffbb33'],['ready','ΕΤΟΙΜΑ ΑΠΟΘΗΚΗΣ','#00C851']].map(([key,label,color]) => (
          <View key={key}>
            <TouchableOpacity style={[styles.listHeader, { backgroundColor:color }]} onPress={() => toggle(key)}><Text style={styles.listHeaderTxt}>● {label} ({sasiOrders.filter(o=>o.status===key.toUpperCase()).length})</Text></TouchableOpacity>
            {expanded[key] && sasiOrders.filter(o=>o.status===key.toUpperCase()).map(o=>renderCard(o))}
          </View>
        ))}
        <TouchableOpacity style={[styles.listHeader, { backgroundColor:'#333', marginTop:20 }]} onPress={() => toggle('archive')}><Text style={styles.listHeaderTxt}>📂 ΑΡΧΕΙΟ ΠΩΛΗΣΕΩΝ ({soldSasiOrders.length})</Text></TouchableOpacity>
