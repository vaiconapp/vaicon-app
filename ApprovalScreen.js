...
export default function ApprovalScreen({ onClose, currentUserName = '', resolveLabel = (u) => u, coatings = [] }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSubs(list.filter(s => s.status === 'PENDING' && s.orderType === 'ΤΥΠΟΠΟΙΗΜΕΝΗ').sort((a, b) => (a.submittedAt || 0) - (b.submittedAt || 0)));
    } catch { setSubs([]); }
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  // Δημιουργία εγκεκριμένης παραγγελίας (1 πόρτα) + δέσμευση στοκ + log. Δεν αγγίζει το order_seq.
  const persistApproved = async (sub, number, id, groupMeta) => {
    ...
    const r = await fetch(`${FIREBASE_URL}/std_orders/${id}.json`, { method: 'PUT', body: JSON.stringify(order) });
    if (!r.ok) throw new Error();

    // Δέσμευση στοκ (ίδια λογική με την κανονική αποθήκευση)
    const isMoni = sub.sasiType === 'ΜΟΝΗ ΘΩΡΑΚΙΣΗ' || !sub.sasiType;
    const reserveSasi = isMoni && !sub.lock && !sub.heightReduction;
    const reserveCase = sub.kypri !== 'ΝΑΙ';
    const qty = parseInt(sub.qty) || 1;
    const newRes = { orderNo: number, customer: sub.customer || '', qty };
    if (reserveSasi) {
      const sk = sasiKey(String(sub.h), String(sub.w), sub.side);
    ...
    if (reserveCase) {
      const ck = caseKey(String(sub.h), String(sub.w), sub.side, sub.caseType);
      const fb = { qty: 0, reservations: [], caseType: (sub.caseType || '').includes('ΑΝΟΙΧΤΟΥ') ? 'ΚΑΣΑ ΑΝΟΙΧΤΗ' : 'ΚΑΣΑ ΚΛΕΙΣΤΗ' };
