...
const LINK_LABELS = { montage: 'Μοντάρ.', kypri: 'Κυπρί', heightReduction: 'Μείωση', hinges3: 'Έξτρα μεντ.', galva: 'Γαλβ.', pihaki: 'Πηχάκι', casePaint: 'Βαφή' };

export default function PricedListScreen({ title = 'ΛΙΣΤΑ', icon = '📦', items = [], setItems, fbNode = 'misc', placeholder = 'π.χ. όνομα είδους...', showFlags = false, isAdmin = false, onClose }) {
  ...
  const printList = () => {
    ...
    const prog = (l) => showFlags ? `<td class="prog">${[l.showStd && 'Τυπ', l.showEid && 'Ειδ'].filter(Boolean).join(' · ') || '—'}</td>` : '';
    const rows = sorted.map((l, i) => `<tr><td>${i + 1}</td><td>${esc(l.name)}${l.link ? ` <span class="lnk">🔗 ${esc(LINK_LABELS[l.link] || l.link)}</span>` : ''}</td>${prog(l)}<td class="p">${l.price ? '€' + esc(l.price) : ''}</td></tr>`).join('');
    const cols = showFlags ? 4 : 3;
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>
      ...
      th,td{border-bottom:1px solid #ddd;padding:5px 6px;font-size:13px}
      th{text-align:left;border-bottom:2px solid #999}
      .p{text-align:right;font-weight:bold;white-space:nowrap}
      .prog{text-align:center;color:#2e7d32;font-weight:bold;white-space:nowrap}
      .lnk{font-size:10px;color:#1565C0}
      @media print{@page{size:A4 portrait;margin:12mm}}
    </style></head><body><h1>${esc(icon)} ${esc(title)} — ΛΙΣΤΑ (${items.length})</h1>
    <table><tr><th>#</th><th>Είδος</th>${showFlags ? '<th class="prog">Πρόγραμμα</th>' : ''}<th class="p">Τιμή</th></tr>${rows || `<tr><td colspan="${cols}">Καμία εγγραφή.</td></tr>`}</table></body></html>`;
    printHTML(html, `VAICON — ${title}`);
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.body}>
        ...
        <View style={styles.toolbar}>
          {isAdmin && (
            <TouchableOpacity style={styles.printBtn} onPress={printList}>
